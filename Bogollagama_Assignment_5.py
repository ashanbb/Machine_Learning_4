...
names = ['Linear_Regression', 'Ridge_Regression', 'Lasso_Regression', 
          'ElasticNet_Regression', 'Random_Forest'] 
...
regressors = [LinearRegression(fit_intercept = SET_FIT_INTERCEPT), 
               ...
               ElasticNet(alpha = 0.1, l1_ratio = 0.5, 
                          ...
                          normalize = False, 
                          random_state = RANDOM_SEED),
               RandomForestRegressor(random_state = RANDOM_SEED, max_features = "log2", bootstrap= True, n_estimators = 100)]


from sklearn.model_selection import KFold

N_FOLDS = 10 # leave one out cross validation N = number of observations 

cv_results = np.zeros((N_FOLDS, len(names))) # array with as many rows as folds and as many columns for names 

ev_results = np.zeros((N_FOLDS, len(names)))
...
for train_index, test_index in kf.split(model_data):
    ...
    for name, reg_model in zip(names, regressors):
        ...
        # evaluate on train set for this fold 
        y_train_predict = reg_model.predict(X_train)
        
        fold_method_r2_result_train = r2_score(y_train, y_train_predict)
        print('Coefficient of determination (R-squared):',fold_method_r2_result_train)
        
        fold_method_result_train = sqrt(mean_squared_error(y_train, y_train_predict))
        print(reg_model.get_params(deep=True))
        print('Root mean-squared error:', fold_method_result_train)
        ...
        ev_results[index_for_fold, index_for_method] = fold_method_result_train
...
print('\nMethod              Root mean-squared error training set', sep = '')
print(ev_results_df.mean())           

print('\nMethod                Root mean-squared error test set', sep ='' )
print(cv_results_df.mean())   
 
print('\nMethod               r-squared test set', sep = '')
print(dv_results_df.mean())   

rnd_clf = RandomForestRegressor(random_state = RANDOM_SEED, max_features = "log2", bootstrap= True, n_estimators = 100)
rnd_clf.fit(X_train, y_train)

print("Random_Forest Feature Importance",rnd_clf.feature_importances_)
